...
| Mode | Storage | Flow |
|------|---------|------|
| **Electron** | macOS Keychain via `safeStorage` | UI → IPC → main process → encrypt → Keychain → inject env vars → restart server |
| **Dev** | `site-config.json` or `.env.local` | UI → API → write to JSON file / env vars read directly |
...
2. **SEO data storage**: Store in `resource_meta` table or add dedicated `seo_data` table?
...
5. **Database location**: Keep in project dir (`./data/`) or move to `~/Library/Application Support/Juggernaut/`?

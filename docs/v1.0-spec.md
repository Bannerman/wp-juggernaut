...
Starting fresh means rewriting **~9,500 lines** and losing 2-3 weeks to get back to the current state. The architecture described in `v1.0recovery.md` is essentially what's already partially built. The plugin type system alone (696 lines of carefully designed interfaces) would take significant effort to recreate.
...
**Status: ✅ Generic.** Uses `getTaxonomies()` from profile. Works with any post type via `fetchAllResources()`.

### 6.3 Push Engine (`push.ts`)

**Status: ✅ Mostly generic.** Uses profile taxonomies. One issue: `TAXONOMY_META_FIELD` mapping imported from MetaBox plugin — should come from profile's `plugin_settings.metabox.taxonomy_meta_mapping` instead.
...
> Verified against codebase as of v0.9.9 (2026-02-06).

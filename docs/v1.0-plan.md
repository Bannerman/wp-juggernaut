...
### Recently Added
- [x] Auto-discover fields from WordPress per post type for field mapping screen
  - `discoverFieldsForPostType()` in `discovery.ts` fetches sample resources + taxonomies
  - `/api/discover-fields` standalone endpoint for field discovery
  - `/api/field-mappings` GET merges discovered fields with profile-defined fields
  - Profile fields take priority (better labels/types); discovered fields supplement
  - Graceful fallback if WordPress is unreachable
...
- **Plexkits profile should be hardcoded/bundled** - not require import
- Profiles location: `src/lib/profiles/` (e.g., `plexkits.json`)
...
- Support for adding entirely different WordPress sites (not just plexkits variants)

### Known Bugs to Fix
- **SEO title edit not persisting**: ~~Changes appear to save in modal but aren't pushed and disappear on reopen~~
  - **Root cause found**: SEO API route was using legacy credentials (not Keychain) - FIXED
  - **Architecture issue**: SEO saves go directly to WordPress, not local DB, so they bypass the dirty flag system
  - **TODO**: Consider saving SEO data to local DB like other fields, then push with rest of changes

### SEO Data Architecture (Current)
- SEO data is fetched directly from WordPress via SEOPress API
- SEO edits are saved directly to WordPress (not local)
- If WordPress auth fails, changes are lost silently (now shows error)
- **Future**: Store SEO in local DB, sync with rest of resource data
